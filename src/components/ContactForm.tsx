...
export default function ContactForm({ inquiry }: ContactFormProps) {
  ...
  // Autofill if user chose an artwork in the Gallery or requested Branding
  useEffect(() => {
    if (inquiry) {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    const typeLabel = inquiryType === 'obra' ? 'Consulta Obras' : inquiryType === 'encargo' ? 'Encargo Custom' : inquiryType === 'branding' ? 'Proyecto Branding' : 'Mensaje General';
    const emailSubject = `Contacto Web - ${name} (${typeLabel})`;
    ...
    try {
      ...
      // Clear all form fields as requested
      setName('');
      setEmail('');
      setMessage('');
      setArtworkInfo('');
          ...
          <div className="p-8 sm:p-12 text-center animate-fade-in font-sans">
            ...
            {submittedLetter?.fallbackNeeded && (
              <div className="mb-10 text-center">
                <a
                  href={`mailto:macatachavalli@gmail.com?subject=${encodeURIComponent('Contacto Web - ' + (submittedLetter?.name || 'Consulta'))}&body=${encodeURIComponent(submittedLetter?.message || '')}`}
